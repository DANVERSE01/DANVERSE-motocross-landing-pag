...
export default function InteractivePortrait() {
  ...
  useEffect(() => {
    ...
    const bgPlaneMaterial = new THREE.MeshBasicMaterial({ color: 0x020814, transparent: true })
    ...
    bgPlaneMaterial.onBeforeCompile = (shader) => {
      ...
      `.replace(
        ...
        vec3 colorBg = vec3(0.008, 0.031, 0.078);
        vec3 colorSoftShape = vec3(0.039, 0.133, 0.22);
        vec3 colorLine = vec3(0.894, 1.0, 0.227);
  ...
  return (
    <div
      ref={containerRef}
      className="fixed inset-0 w-full h-full bg-dan-bg cursor-crosshair overflow-hidden"
      style={{ touchAction: "none" }}
    >
      <div className="absolute bottom-4 left-4 z-10 pointer-events-none glass-card px-3 py-2 rounded-lg">
        <span className="text-dan-acid text-xs font-bold tracking-wider">AI-POWERED CREATIVE STUDIO</span>
      </div>
    </div>
  )
}
